...
import com.coretec.sensing.databinding.ActivityCustomMapBinding;
import com.coretec.sensing.databinding.ContentCustomMapBinding;
...
public class CustomMapActivity extends AppCompatActivity implements OnTouchMapListener, NavigationView.OnNavigationItemSelectedListener, View.OnClickListener {

    private ActivityCustomMapBinding activityBinding;
    private ContentCustomMapBinding contentBinding;
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        switch (id) {
            ...
            case R.id.nav_setting_paramter:
                showSettingParameterDialog(CustomMapActivity.this);
                return false;

            case R.id.nav_setting_logging:
                showSettingLoggingDialog(CustomMapActivity.this);
                return false;
        }
        return true;
    }
        ...
        switch (id) {
            ...
            case R.id.item_edit:
                ...
                if (isEdit) {
                    ...
                    CsvUtil.writeApCsv(new ArrayList<>(apHashMap.values()), MAPPath);
                    CsvUtil.writePoiCsv(poiArrayList, MAPPath);
                    CsvUtil.writeNodeCsv(nodeArrayList, MAPPath);
                    CsvUtil.writeLinkCsv(linkArrayList, MAPPath);
                }
                break;
    ...
    private void downloadDB() {
        FilePath.setDbName("custom.db");

        PrefManager pref = new PrefManager(this);
        if (!pref.isDownloadDB("custom")) {
            DBDownload.copyDB(pref, this, "custom");

            CsvUtil.writeCustomMapSetting();

            Bitmap bitmapImage = BitmapFactory.decodeResource(getResources(), R.drawable.bg_map3);
            bitmapImage = Bitmap.createScaledBitmap(bitmapImage, 2411, 2040, true);

            File mypath = new File(MAPPath, "bg_map.png");
        ...
        PermissionListener permissionlistener = new PermissionListener() {
            ...
            public void onPermissionGranted() {
                if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_WIFI_RTT)) {
                    Toast.makeText(CustomMapActivity.this, "RTT를 지원하지 않아 일부 기능 사용 시 앱이 종료됩니다.", Toast.LENGTH_SHORT).show();
//            finish();
                }
    ...
    private void init() {
        //데이터 바인딩 초기화
        activityBinding = DataBindingUtil.setContentView(this, R.layout.activity_custom_map);
...
//        contentBinding.imgMarker.removeAllViews();
        ...
        File poiFile = new File(MAPPath, "POI.csv");
        File apFile = new File(MAPPath, "AP.csv");
        File linkFile = new File(MAPPath, "LINK.csv");
        File nodeFile = new File(MAPPath, "NODE.csv");
    ...
    private void showMedianLocationView() {
        ...
        myMedianLocationView = new MoveImageView(contentBinding.imgMap.savedMatrix2, parentWidth, parentHeight, (int) (medianLocation.getX() * PIXEL_PER_METER_WIDTH + LEFT_BLANK_PIXEL), (int) (MAP_HEIGHT - (medianLocation.getY() * PIXEL_PER_METER_HEIGHT) - BOTTOM_BLANK_PIXEL), CustomMapActivity.this);
    ...
    private void showDbScanLocationView() {
        ...
        myDbScanLocationView = new MoveImageView(contentBinding.imgMap.savedMatrix2, parentWidth, parentHeight, (int) (dbScanLocation.getX() * PIXEL_PER_METER_WIDTH + LEFT_BLANK_PIXEL), (int) (MAP_HEIGHT - (dbScanLocation.getY() * PIXEL_PER_METER_HEIGHT) - BOTTOM_BLANK_PIXEL), CustomMapActivity.this);
    ...
    private void showReliabilityLocationView() {
        ...
        myReliabilityLocationView = new MoveImageView(contentBinding.imgMap.savedMatrix2, parentWidth, parentHeight, (int) (reliabilityLocation.getX() * PIXEL_PER_METER_WIDTH + LEFT_BLANK_PIXEL), (int) (MAP_HEIGHT - (reliabilityLocation.getY() * PIXEL_PER_METER_HEIGHT) - BOTTOM_BLANK_PIXEL), CustomMapActivity.this);
    ...
    private void showCorrectionLocationView() {
        ...
        myNearLocationView = new MoveImageView(contentBinding.imgMap.savedMatrix2, parentWidth, parentHeight, (int) (correntionLocation.getX() * PIXEL_PER_METER_WIDTH + LEFT_BLANK_PIXEL), (int) (MAP_HEIGHT - (correntionLocation.getY() * PIXEL_PER_METER_HEIGHT) - BOTTOM_BLANK_PIXEL), CustomMapActivity.this);
...
//    ===========================================================================================
    ...
    public void onClick(View v) {
        ...
        switch (resId) {
            case R.id.btnFind:
                Intent intent = new Intent(CustomMapActivity.this, LocationActivity.class);
...
//        bluetoothLeScanner.startScan(leScanCallback);
    ...
    public void scanWifi() {
        ...
        if (!isLogging)
            LoadingDialog.showDialog(CustomMapActivity.this, "AP를 스캔 중입니다...");
    }
